<template>
  <li class="list-none">
    <div
      class="mx-auto m-4 border rounded-lg overflow-hidden font-sans"
      :class="severityClasses.border"
    >
      <div
        @click="isOpen = !isOpen"
      ...
      >
        <div class="flex items-center gap-3">
          <div
            class="p-1 rounded shadow-sm text-white flex items-center justify-center"
          ...
          <div>
            ...
            <p class="text-xs text-gray-500 text-left capitalize">
      ...
      <div
        v-if="isOpen"
        class="p-4 border-t text-sm text-gray-800 space-y-4 text-left"
      ...
      >
        ...
        <div class="pt-2">
          ...
          <div class="flex items-center gap-2">
            ...
            <span
              >{{ warning.severity.charAt(0).toUpperCase() + warning.severity.slice(1) }}: danger
              level {{ warning.level }} - {{ warning.title.toLowerCase() }}</span
            >
          </div>
        </div>
      </div>
...
const warnings = ref(
  Array(5).fill({
    ...
    description:
      'On Sunday we expect considerable avalanche danger for region Nordenskiöld Land. Avalanche problem is persistent weak layer...',
