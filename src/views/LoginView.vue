...
<template>
  <div class="min-h-screen flex items-center justify-center bg-[#f8fafc] p-6 font-sans">
    <div
      class="max-w-7xl h-180 w-full bg-white flex flex-col md:flex-row rounded-[40px] shadow-xl overflow-hidden border border-gray-100"
    >
      <div class="w-full md:w-1/2 p-10 lg:p-20">
        ...
        <div class="max-w-md mx-auto">
          ...
          <form @submit.prevent="signup" class="flex flex-col gap-4 pt-5">
            <div class="mb-6">
              <label class="block text-sm font-semibold text-gray-400 mb-2 ml-2"> Full Name </label>
              <input
                v-model="name"
                type="text"
            ...
            <div class="mb-6">
              <label class="block text-sm font-semibold text-gray-400 mb-2 ml-2"> Password </label>
              <input
                v-model="password"
                type="password"
                placeholder="Comfire a password"
                class="w-full px-5 py-4 rounded-xl bg-gray-50 border border-gray-100 focus:bg-white focus:ring-2 focus:ring-blue-50 transition-all outline-none text-gray-600 placeholder:text-gray-300"
                required
              />
            </div>
            ...
            <button
              class="w-full bg-[#90E0EF] text-white py-5 rounded-xl font-bold text-lg shadow-lg hover:brightness-105 transition-all active:scale-[0.98]"
            >
              Sign Up
            </button>
          </form>
          ...
          <p class="text-center text-sm mt-10 text-gray-400 pt-3">
            ...
            <RouterLink to="/Signup" class="text-blue-600 font-bold hover:underline ml-1"
